...
public class EquService extends BaseService  {
	public static DataGrid getEqus(Map prams){
		...
		String condtion  = convertSearchMapToSql("integrity.monitor.equ", prams);
		//拼凑order by
		String torderby = convertToSqlOrderBy(sort,order);
		if(!StringUtil.isBlank(torderby)){
			condtion += " order by"+ torderby;
		}
		String[] tsqlary = StringUtil.split(SqlKit.sql("integrity.monitor.findEqu.pageSql"), "{|}"); 
		Page<EquipStrategy> tpages =  EquipStrategy.dao.paginate(page, rows,tsqlary[0], tsqlary[1]+" where 1=1 "+ condtion);

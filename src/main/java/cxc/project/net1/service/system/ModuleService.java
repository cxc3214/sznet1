...
import cxc.project.net1.model.Modules;
...
public class ModuleService extends BaseService  {
	public static DataGrid getModules(Map prams){
		...
		String condtion  = convertSearchMapToSql("system.module", prams);
		//拼凑order by
		String torderby = convertToSqlOrderBy(sort,order);
		if(!StringUtil.isBlank(torderby)){
			condtion += " order by"+ torderby;
		}
		String[] tsqlary = StringUtil.split(SqlKit.sql("system.findModule.pageSql"), "{|}"); 
		Page<Modules> tpages = Modules.dao.paginate(page, rows,tsqlary[0], tsqlary[1]+" where 1=1 "+ condtion);

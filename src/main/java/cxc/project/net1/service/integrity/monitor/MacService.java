...
public class MacService extends BaseService  {
	public static DataGrid getMacs(Map prams){
		...
		String condtion  = convertSearchMapToSql("integrity.monitor.mac", prams);
		//拼凑order by
		String torderby = convertToSqlOrderBy(sort,order);
		if(!StringUtil.isBlank(torderby)){
			condtion += " order by"+ torderby;
		}
		String[] tsqlary = StringUtil.split(SqlKit.sql("integrity.monitor.findMac.pageSql"), "{|}"); 
		Page<MacAddress> tpages =  MacAddress.dao.paginate(page, rows,tsqlary[0], tsqlary[1]+" where 1=1 "+ condtion);

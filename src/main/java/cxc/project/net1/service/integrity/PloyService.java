package cxc.project.net1.service.integrity;
...
import cxc.project.net1.model.Strategy;
import cxc.project.net1.model.web.DataGrid;
import cxc.project.net1.service.BaseService;

public class PloyService extends BaseService  {
	public static DataGrid getPloys(Map prams){
		...
		String condtion  = convertSearchMapToSql("integrity.ploy", prams);
		//拼凑order by
		String torderby = convertToSqlOrderBy(sort,order);
		if(!StringUtil.isBlank(torderby)){
			condtion += " order by"+ torderby;
		}
		String[] tsqlary = StringUtil.split(SqlKit.sql("integrity.findPloy.pageSql"), "{|}"); 
		Page<Strategy> tpages =  Strategy.dao.paginate(page, rows,tsqlary[0], tsqlary[1]+" where 1=1 "+ condtion);

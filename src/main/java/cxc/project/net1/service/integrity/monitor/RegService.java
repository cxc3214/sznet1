...
public class RegService extends BaseService  {
	public static DataGrid getRegs(Map prams){
		...
		String condtion  = convertSearchMapToSql("integrity.monitor.reg", prams);
		//拼凑order by
		String torderby = convertToSqlOrderBy(sort,order);
		if(!StringUtil.isBlank(torderby)){
			condtion += " order by"+ torderby;
		}
		String[] tsqlary = StringUtil.split(SqlKit.sql("integrity.monitor.findReg.pageSql"), "{|}"); 
		Page<RegStrategy> tpages =  RegStrategy.dao.paginate(page, rows,tsqlary[0], tsqlary[1]+" where 1=1 "+ condtion);

...
public class ChangeService extends BaseService  {
	public static DataGrid getChanges(Map prams){
		...
		String condtion  = convertSearchMapToSql("integrity.change", prams);
		//拼凑order by
		String torderby = convertToSqlOrderBy(sort,order);
		if(!StringUtil.isBlank(torderby)){
			condtion += " order by"+ torderby;
		}
		String[] tsqlary = StringUtil.split(SqlKit.sql("integrity.findChange.pageSql"), "{|}"); 
		Page<HIPSLog> tpages =  HIPSLog.dao.paginate(page, rows,tsqlary[0], tsqlary[1]+" where 1=1 "+ condtion);
